...
void Level5Scene::onEnter(sf::RenderWindow& win)
{
    if (!hasEntered)
    {
        ...
        cpu.init(
            "assets/images/level5_cpu_symbol.png",
            "assets/images/level5_cpu_picture.png",
            "CPU"
        );

        cpu.setStrategy(new Level5Strategy()); //  Level 5 strategy
...
void Level5Scene::handleEvent(const sf::Event& event, AudioManager& audio)
{
    ...
    if (!gameStarted)
    {
        ...
        if (gridSizeInput.isConfirmed())
        {
            ...
            audio.getLevel5GameAudio().play();
            gameStarted = true;
            cursorRow = 0;
            cursorCol = 0;
            std::cout << "Level 5 started with grid size: " << size << "\n";
        }
    }
    else
    {
        ...
        if (!gameManager.isGameOver())
        {
            ...
            else if (key->scancode == sf::Keyboard::Scancode::Enter)
            {
                ...
                if (valid) // ✅ save AFTER CPU move — playerTurn is now true
                {
                    SaveData data;
                    data.level = 5; // change per level
...
void Level5Scene::draw(sf::RenderWindow& window, AudioManager& audio)
{
    onEnter(window);
...
void Level5Scene::fullReset()
{
    ...
    cursorRow = 0;
    cursorCol = 0;
    std::cout << "Level5Scene fully reset\n";
}
void Level5Scene::loadSave(const SaveData& data, sf::RenderWindow& win, AudioManager& audio)
{
    ...
    std::cout << "Level5Scene save loaded\n";
}