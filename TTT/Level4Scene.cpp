...
void Level4Scene::onEnter(sf::RenderWindow& win)
{
    if (!hasEntered)
    {
        window = &win;
        ...
        cpu.init(
            "assets/images/level4_cpu_symbol.png",
            "assets/images/level4_cpu_picture1.png",
            "CPU"
        );

        cpu.setStrategy(new Level4Strategy()); //  Level 4 strategy
...
void Level4Scene::handleEvent(const sf::Event& event, AudioManager& audio)
{
    ...
    if (!gameStarted)
    {
        ...
        if (gridSizeInput.isConfirmed())
        {
            ...
            cursorRow = 0;
            cursorCol = 0;
            std::cout << "Level 4 started with grid size: " << size << "\n";
        }
    }
    else
    {
...
void Level4Scene::draw(sf::RenderWindow& window, AudioManager& audio)
{
    onEnter(window);
